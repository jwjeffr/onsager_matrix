"""
Library for computing the Onsager matrix using the generalized Einstein formula
"""
...
def total_displacement_modifier(frame: int, data: ovito.data.DataCollection) -> None:
    ...
    """
    OVITO modifier for computing total displacement of all types
    :param frame: frame to modify
    :param data: data collection to modify
    :return:
    """

    # get types and displacements, assume user has already added displacement modifier
    types = data.particles['Particle Type'][...]
    per_particle_displacements = data.particles['Displacement'][...]

    # store total displacements in data collection
    for i in set(types):

        displacement_array = per_particle_displacements[types == i]
        data.attributes[f'Total displacement {i:.0f}'] = np.sum(displacement_array, axis=0)
...
class OnsagerMatrix:
    ...
    def __getitem__(self, item):
        ...
        if item is Ellipsis:
            ...
            raise ValueError('only subscriptable with Ellipses or indices')
